...
GameEngine::GameEngine()
    : m_window(sf::VideoMode(WINDOW_SIZE,WINDOW_SIZE),"TicTacToe vs Medium Bot"),
    m_board(),
    m_renderer(m_window),
    m_isPlayer1Turn(true),
    m_isGameOver(false){
    m_player1 = std::make_unique<HumanPlayer>(Cell::X, "You");
    m_player2 = std::make_unique<MediumBot>(Cell::O, "Medium Bot");
}
...
void GameEngine::update(){
    ...
    if (m_isPlayer1Turn == false){
        auto bot = dynamic_cast<MediumBot*>(m_player2.get());
        if (bot){
            Move m = bot->getSmartMove(m_board);
